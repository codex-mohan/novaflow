...
export class ConversationDatabase {
    ...
    public static async initialize(db_path: string): Promise<void> {
        ...
        await Database.initialize(db_path, "novaflow", "conversations");
        ...
        await db.query("DEFINE INDEX idx_title ON conversation FIELDS title UNIQUE");
        ...
        await db.query("DEFINE FIELD conversation_id ON message TYPE record<conversation>;");
        ...
        console.log("Schema definitions for `conversation` and `message` are set up.");
    }

    /**
     * Create a new conversation and return its ID
     */
    public static async createConversation(user_id: string, title: string): Promise<string> {
        const db = Database.getConnection();
     ...
     */
    public static async addMessage(conversation_id: string, message: Message): Promise<void> {
        const db = Database.getConnection();
     ...
     */
    public static async getConversationMessages(conversation_id: string): Promise<Message[]> {
        const db = Database.getConnection();
     ...
     */
    public static async getUserConversations(user_id: string): Promise<Conversation[]> {
        const db = Database.getConnection();
